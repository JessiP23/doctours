...
Scope of this document: everything needed to ship **Level 0, 100% working**, on the given stack (Next.js 16 / Claude / Supabase / Vercel / Sabre sandbox), in a shape that Levels 1–3 can grow into without rewrites. Nothing here is Level 1+ work; where a decision is made _because_ of a later level, it is flagged as such.
...
| Hard dates respected; landing after deadline = failed booking           | `validateItinerary()` filters offers _before_ the model sees them and re-checks _before_ `create_order`. Both layers.                  | `lib/trip/validate.ts` + unit tests |
...
| Validation of dates/times | **`luxon`** (or Temporal polyfill)                                                                                                | Every hard deadline is _local time in Istanbul_. You need real IANA zone math (`Europe/Istanbul`, `America/New_York`). Never compare naive strings.                                                                                                                                                              |
...
- **`tool_choice: 'any'`** plus a terminal **`reply` tool** means the model must _always_ end the turn with structured bubbles. This is how you get "no blobs, no markdown" reliably instead of hoping the prompt is followed. Plain-text responses are never rendered; if one occurs (it won't with `any`), it's converted through `humanize()` as a safety net.
...
### 6.3 System prompt (short, and mostly _state_, not _rules_)
...
- Server returns `{bubbles: string[]}`. Client shows a typing indicator, then reveals bubble _i_ after `min(300 + 28 × chars, 1800)` ms. No streaming; nothing arrives mid-sentence.
...
7. Hotel nights = arrival date → return departure date, derived from the _booked_ flight, never assumed.
