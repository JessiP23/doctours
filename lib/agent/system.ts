...
export function buildSystemPrompt(state: TripState, now = new Date()): string {
  ...
  return [
    ...
    `This trip is booked for ${r.adults === 1 ? 'exactly one traveller' : `exactly ${r.adults} travellers`}: the patient${r.adults === 1 ? '' : ' and their party'}. You cannot add a companion, buy a second ticket, or add a guest to a room. If they mention travelling with someone, say plainly that you can only arrange their own travel and that anyone else needs their own booking — then carry on with theirs. A room's bed count and its maximum occupancy describe the room, not permission: never imply a second person is covered by a booking they are not on.`,
