...
export function buildSystemPrompt(state: TripState, now = new Date()): string {
  ...
  return [
    ...
    `Fares are held for about twenty minutes. If a booking comes back expired the tool re-checks the same flights and gives you the current price — confirm that new price before booking it. If a tool returns an error, say what happened in plain words and offer the next step.`,
    ``,
    `OUTPUT CONTRACT`,
